...
def check():
    ...
    model = nn.Linear(64,64).cuda().bfloat16()
    modelhijack = nn.Linear(64,64).cuda().bfloat16()
    ...
    input_gemv_3dim = torch.randn(1, 1, 64, generator=generator, device='cuda').bfloat16()
    input_gemv = torch.randn(1, 64, generator=generator, device='cuda').bfloat16()
    input_gemm_3dim = torch.randn(1, 2, 64, generator=generator, device='cuda').bfloat16()
    ...
    print(f"output_gemm_3dim_h: {output_gemm_3dim_hijack}",output_gemm_3dim_hijack.shape)
